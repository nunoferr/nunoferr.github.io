  ...
  class VillagesTroopsCounter {
    static VillagesTroopsCounterTranslations() {
      return {
        en_US: {
          ...
          errorMessages: {
            premiumRequired:
              "Error. A premium account is required to run this script!",
            errorFetching:
              "An error occured while trying to fetch the following URL:",
            missingSavengeMassScreenElement:
              "An error occurred trying to located the ScavengeMassScreen element inside the mass scavenge page.",
          },
        ...
        pt_PT: {
          ...
          errorMessages: {
            premiumRequired:
              "Erro. É necessário possuir conta premium para correr este script!",
            errorFetching: "Ocorreu um erro ao tentar carregar o seguinte URL:",
            missingSavengeMassScreenElement:
              "Ocorreu um erro ao tentar localizar o elemento ScavengeMassScreen dentro da página de buscas em massa.",
          },
          successMessage: "Carregado com sucesso!",
          loadingMessage: "A carregar...",
          loadingWorldConfigMessage: "A carregar configurações do mundo...",
          credits:
            "Contador de tropas em casa e em buscas v1.2.0 por NunoF- (.com.pt)",
        },
      };
    }
    ...
    constructor() {
      this.UserTranslation =
        game_data.locale in
        VillagesTroopsCounter.VillagesTroopsCounterTranslations()
          ? (this.UserTranslation =
              VillagesTroopsCounter.VillagesTroopsCounterTranslations()[
                game_data.locale
              ])
      ...
      this.availableSupportUnits = Object.getPrototypeOf(
        this.availableSupportUnits
      );
      this.availableSupportUnits.splice(
        this.availableSupportUnits.indexOf("militia"),
        1
      );
    ...
    #fetchHtmlPage(url) {
      ...
      $.ajax({
        ...
        error: function (jqXHR, textStatus, errorThrown) {
          console.log(jqXHR);
          UI.ErrorMessage(
            `${this.UserTranslation.errorMessages.errorFetching} ${url}`
          );
        },
      });
      return temp_data;
    ...
    async #getTroopsScavengingWorldObj() {
      ...
      do {
        var scavengingObject = await getScavengeMassScreenJson(
          this,
          currentPage,
          lastRunTime
        );
        ...
        $.each(scavengingObject, function (id, villageData) {
          ...
          $.each(villageData.options, function (id, option) {
            if (option.scavenging_squad !== null) {
              $.each(
                option.scavenging_squad.unit_counts,
                function (key, value) {
                  if (key !== "militia")
      ...
      async function getScavengeMassScreenJson(
        currentObj,
        currentPage = 0,
        lastRunTime = 0
      ) {
        await currentObj.#waitMilliseconds(lastRunTime, 200);
        var html = currentObj.#fetchHtmlPage(
          currentObj.#generateUrl("place", "scavenge_mass", {
            page: currentPage,
          })
        );
        var matches = html.match(
          /ScavengeMassScreen[\s\S]*?(,\n *\[.*?\}{0,3}\],\n)/
        );
        if (matches.length <= 1) {
          UI.ErrorMessage(
            this.UserTranslation.errorMessages.missingSavengeMassScreenElement
          );
          return false;
        }
        matches = matches[1];
    ...
    async #getTroopsNonScavengingWorldObj() {
      ...
      do {
        ...
        var lastVillageIdTemp = $(troopsTable)
          .find("span")
          .eq(0)
          .attr("data-id");
        if (lastVillageId !== null && lastVillageId === lastVillageIdTemp)
          break; // an exact multiple of 1000 villages exist (i.e. 1000, 2000, 3000) and we are re-reading the last page.
        ...
        $.each(troopsTable, function (id, tbodyObj) {
          ...
          $.each(currentObj.availableSupportUnits, function (key, value) {
            troopsObj.villagesTroops[value] += parseInt(
              villageTroopsLine.eq(c).text().trim()
            );
            c++;
          });
        });
    ...
    #getGroupsObj() {
      var html = $.parseHTML(
        this.#fetchHtmlPage(
          this.#generateUrl("overview_villages", "groups", { type: "static" })
        )
      );
      ...
      if ($(groups).length > 0) {
        $.each(groups, function (id, group) {
          ...
          groupsArr[group.getAttribute("data-group-id")] = val.substring(
            1,
            val.length - 1
          );
        });
      } else {
        // User has more than 5 groups
        groups = $(html).find(".vis_item select option");
        $.each(groups, function (id, group) {
          groupsArr[new URLSearchParams($(group).val()).get("group")] = $(group)
            .text()
            .trim();
        });
      }
    ...
    async #createUI() {
      ...
      var troopsObj = this.isScavengingWorld
        ...
        <table id="support_sum" class="vis overview_table" width="100%">
            ...
            <tbody>
                ...
                ${
                  this.isScavengingWorld
                    ? getTroopsLine(
                        this.UserTranslation.scavenging,
...
<!---br--->
<br>
<span style="text-decoration: bold;font-weight: bold;font-size: 10px;">${
...
`;
      ...
      function getGroupsHtml(objInstance) {
        ...
        return (
          '<select onchange="villagesTroopsCounter.changeGroup(this)">' +
          html +
          "</select>"
        );
      }
    ...
    changeGroup(obj) {
      this.#fetchHtmlPage(
        this.#generateUrl("overview_villages", null, { group: obj.value })
      );
