/*
* Script Name: Tribe Armies Finder
* Version: v1.0.0
 ...
 * This script can NOT be cloned and modified without permission from the script author.
 ...
 if (typeof tribeArmiesFinder !== 'undefined') {
    tribeArmiesFinder.init();
 } else {
 class TribeArmiesFinder {
    static TribeArmiesFinderTranslations() {
        return {
            en_US: {
                ...
                credits: 'Tribe Armies Finder script v1.0.0 by NunoF- (.com.pt)'
            },
            pt_PT: {
                ...
                credits: 'Procurar exércitos da tribo v1.0.0 por NunoF- (.com.pt)'
            }
        };
    }
    ...
    constructor() {
        ...
        UnitPopup.whenDataReady(function() { tribeArmiesFinder.init() }); // fetch units data for units automatic translations
    }
    ...
    async #createUI(runFinder = false) {
        ...
        <div id="tribeArmiesFinder" class="${this.isMobile ? 'mobile-app' : ''}">
            <form onsubmit="tribeArmiesFinder.calculate(event);">
        ...
        function createArmiesSection(UserTranslation, armies) {
            ...
            <table class="vis armiesSection">
                <tbody>
                    <tr>
                        ...
                        <th>${UserTranslation.table.counter}</th>
            ...
            $.each(armies.armiesContent, function(username, content) {
                ...
                <tr>
                    ...
                    <td class="bold">${Object.keys(content.villages).length}</td>
            ...
            return html + `
                <tr>
                    <td class="bold">${UserTranslation.table.total}</td>
                    <td class="bold">${armies.armiesCount}</td>
                </tr>
            </tbody>
        </table>
        ...
        $('.armiesSection').eq(0).css('max-width', $('#tribeArmiesFinder').css('width'));
        ...
        function createStyleElement() {
            ...
            #tribeArmiesFinder .searchTable td, #tribeArmiesFinder .armiesSection td a {
                white-space: nowrap;
            }

            #tribeArmiesFinder .armiesSection tr td:not(:last-child) {
                text-align: center;
            }

            #tribeArmiesFinder .armiesSection .bold {
                font-weight: bold;
            }
    ...
    async #getTribeArmies(unitsFormValues) {
        ...
        Dialog.close();
        Dialog.show('import', `<h2 id="tribeArmiesLoading" style="color:green;">${this.UserTranslation.loadingMessage}</h2><div id="tribeArmiesFinderLoadingBar" class="progress-bar live-progress-bar"><div style="background: rgb(146, 194, 0);"></div><span class="label" style="margin-top:0px;"></span></div>`, Dialog.close());
        UI.InitProgressBars()
        UI.updateProgressBar($('#tribeArmiesFinderLoadingBar'), 0, Object.keys(users).length);
        ...
        for(var username in users)
        {
            ...
            UI.updateProgressBar($('#tribeArmiesFinderLoadingBar'), c + 1, Object.keys(users).length);
            ...
            $.each(availableSupportUnits, function(key, value) {
                ...
                <td>
                    <div class="searchCheckboxField">
                        <input type="checkbox" style="vertical-align:${!isNaN(unitsFormValues[value]['smaller']) ? '8px' : !isMobile ? 'baseline' : 'middle'};${this.isMobile ? 'width: 13px;height: 13px;' : ''}" ${!isNaN(unitsFormValues[value]['smaller']) ? 'checked' : ''} onclick="tribeArmiesFinder.changeCheckedStatus(this);">
                    </div>
    ...
    #createGroupsSection() {
        ...
        <div id="groupsSection" style="border: 1px solid #7d510f;display: block;width: 291px;margin-top:10px;padding:5px;background: transparent url(https://dspt.innogamescdn.com/asset/86f7f6ca/graphic/index/iconbar-mc.png) scroll left top repeat;">
            ...
            <input type="button" style="margin-top: 10px;" class="btn evt-confirm-btn btn-confirm-yes" value="${this.UserTranslation.groupsSection.createGroupBtn}" onclick="tribeArmiesFinder.createNewGroup()">
            ...
            <input type="button" class="btn btn-default" style="background: #4040f7;" value="${this.UserTranslation.groupsSection.editGroupBtn}" onclick="tribeArmiesFinder.editGroup()">
            <input type="button" class="btn btn-default" value="${this.UserTranslation.groupsSection.deleteGroupBtn}" onclick="tribeArmiesFinder.deleteGroup()">
        </div>
        `;
        ...
        function initEditGroupsSection(UserTranslation, tribeArmiesGroups) {
            ...
            html = `<select id="tribeArmiesGroupsSelect" style="width: 135px;" onchange="tribeArmiesFinder.fillArmies()">${html}</select>`;
            return html;
        }
    }
...
var tribeArmiesFinder = new TribeArmiesFinder();
}