    ...
    async #createUI(runFinder = false) {
        ...
        function createArmiesSection(UserTranslation, availableSupportUnits, armies) {
            ...
            $.each(armies.armiesContent, function(username, content) {
                html += `
                <tr>
                    <td rowspan="${Object.keys(content.villages).length + 1}"><a href="/game.php?screen=info_player&id=${content.playerId}" target="_self">${username}</a></td>
        ...
        `;
            ...
            function printPlayerVillages(villages) {
                ...
                $.each(villages, function(villageCoords, troops) {
                    html += `<tr><td><a target="_self" href="/game.php?screen=info_village&id=${troops['villageId']}">${villageCoords}</a></td>`;
                    $.each(troops, function(troopName, troop) {
